...
\begin{proof}
	For (a), use the fact that $\operatorname{epi}(f)$ is epigraph-like and closed.	For (b), use $V_{f,u}=\operatorname{epi}(f)_{:,u}=\operatorname{epi}(f)\cap \left(\mathbb{R}^m\times \{u\}\right)$ and \Cref{prop:013-yoga-recession} (giving $R_{V_{f,u}}=R_{\operatorname{epi}(f)}\cap \mathbb{R}^m\times\{0\})$. For (c), we have by (a) and (b) that:
		\[
			R_f=r_f^{-1}\left(-\mathbb{R}_{+}\right),\;
