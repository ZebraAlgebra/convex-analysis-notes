...
\begin{coro}[Extended Slater's Condition]\label{coro:044-slater-combined}
	Suppose in \Cref{defn:040-opt-prob} that $g(x)=(g^{(1)}(x),g^{(2)}(x))$ where $g^{(i)}:\mathbb{R}^m\to \mathbb{R}^{r_i}$, with $g^{(1)}$ having polyhedral component functions, $g^{(2)}$ has convex component functions, and assume that the following holds:
...
\paragraph{}We also derive Fenchel duality and conic duality as corollaries to the convex programming duality.
...
\begin{coro}[Conic Duality]\label{coro:044-conic}
	...
	\[
		...
		\end{matrix}
	\]
	by applying Fenchel's duality to $f^{(1)}=f$, $f^{(2)}=\delta_C$, $m=l$, $A=I_m$, noting $f^{(2),\star}=\delta_{C^\ast}$; see \Cref{defn:025-indicator-function}.
