...
\paragraph{}Recall that one of the question we posted in \Cref{sect:042} asks whether $Q^\ast,F^\ast$ are nonempty. The case of $Q^\ast$ (dual reachability) can be obtained in some cases via the Slater's conditions or complementary slackness. The case of $F^\ast$ as mentioned before is more intricate. In the case of linear programs, we have given one such result in \Cref{prop:043-linear-reachability}: when $f^\ast\in \mathbb{R}$, $F^\ast\neq\emptyset$. In this section, we extend both the proof and the result of \Cref{prop:043-linear-reachability}, to derive the fundamental theorem of linear programming. We also give a condition where primal optimal set is nonempty (or compact) via recession cones in the unconstrained case. Another result is postponed to \Cref{coro:047-convex-programming-primal-reachability}, which uses the theory of partial minimization, given in \Cref{sect:047}.
...
\begin{prop}[Unconstrained Case - Primal Optimal set via Recession Cones]\label{prop:045-primal-reachability}
	Suppose in \Cref{defn:040-opt-prob}, $X,f$ are closed convex, $g$ is empty, and that $F\neq\emptyset$.
	\begin{enumerate}[label=(\alph*)]
		\item If $R_X\cap R_f=L_X\cap L_f$, then $F^\ast\neq\emptyset$, and $F^\ast=L_X\cap L_f+X'$ with $X'$ compact.
		\item If $R_X\cap R_f\subset L_f$ with $F$ polyhedral, then $F^\ast\neq\emptyset$.
	\end{enumerate}
	Notable cases that will imply (a): when $f$ has a nonempty compact level set or when $\operatorname{dom}(f)$ is bounded.
\end{prop}

\begin{proof}
	These follows from applying \Cref{coro:014-decomp-compact} (for (a)) or \Cref{prop:014-non-empt-II} (for (b)), with the nested sequence of closed convex sets $\{C_i\}_{i=1}^{\infty}$ given by $C_i=X\cap V_{f,\gamma_i}$ where $\{\gamma_i\}_{i=1}^{\infty}\to f^{\ast,+}$.
\end{proof}
