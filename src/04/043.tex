\section{Convex Optimization Duality - Case of Linear Programs}
\label{sect:043}
...
\begin{prop}[Linear Programming Duality]\label{prop:042-linear-strong-duality}For linear programs, we have the following:
...
\begin{proof}
	(a) is given before. For (b), we show that if $f^\ast\in \mathbb{R}$, then \Cref{prop:042-complementary-slackness} is satisfied; the proof for $q^\ast$ is similar. We have $F\neq \emptyset$. By \Cref{prop:042-linear-reachability}, we have some $x_0\in F^\ast$. We will construct a particular $y_0\in Q$ using linear Farkas lemma, then use complementary slackness to show strong duality. Define:
	\[
		J=\{j:a_j^Tx_0=b_j\}
	\]
