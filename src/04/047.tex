...
\begin{proof}
	Write $\pi:\mathbb{R}^p\times\mathbb{R}^q\times\mathbb{R}\to\mathbb{R}^q\times\mathbb{R}$ as the projection map $(x,y,u)\mapsto(y,u)$. Since $\operatorname{epi}(f)_{y,:}=\operatorname{cl}((\pi\operatorname{epi}(F))_{y,:})$, $f$ is closed if $\pi\operatorname{epi}(F)$ is closed. We show this is the case by \Cref{coro:014-closed-affine}. Firstly:
	\[
